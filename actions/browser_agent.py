...
language browser tasks into a small set of reusable browser primitives and lets
Playwright inspect the current DOM before acting.
"""
...
class BrowserAgent:
    ...
    def execute(self, text: str) -> BrowserTaskResult:
        ...
        if result.handled:
            return result
        result = self._interaction(original, q)
        if result.handled:
            return result
        return BrowserTaskResult(False)
    ...
    def _search(self, q: str) -> BrowserTaskResult:
        match = re.match(
            r"^(?:please\s+)?(?:search|look\s+up|find)\s+(?:for\s+)?(.+?)"
            r"(?:\s+(?:on|in)\s+(?:this\s+|my\s+)?([a-z0-9.-]+)(?:\s+tab)?)?\s*$",
            q,
            re.I,
        )
        if not match:
        ...
        site_raw = (match.group(2) or "").lower().strip()
        site = self.SITE_ALIASES.get(site_raw)
        if not term:
            return BrowserTaskResult(False)
    ...
    def _media(self, q: str) -> BrowserTaskResult:
        # Generic media intent. The topic is user-provided; no channel/video names
        # are embedded in code. "its" uses the current page/search context.
    ...
    def _interaction(self, original: str, q: str) -> BrowserTaskResult:
