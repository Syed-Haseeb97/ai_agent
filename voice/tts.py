...
class TTS:
    ...
    def _load_voice() -> str | None:
        try:
            path = Path(__file__).resolve().parent.parent / "preferences.json"
    ...
    def speak(self, text: str) -> bool:
        ...
        if self._edge_available:
            try: return self._speak_edge(text)
    ...
    def _speak_pyttsx3(self, text: str) -> bool:
        try:
            import pyttsx3
            engine = pyttsx3.init(); engine.setProperty("rate", 185); engine.say(text); engine.runAndWait(); engine.stop()
            return not self._stop_event.is_set()
